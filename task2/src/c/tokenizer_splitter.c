...
static bool is_unicode_letter(unsigned int cp) 
{
    ...
    if (cp >= 0x00C0 && cp <= 0x052F) return true;
    if (cp >= 0x0590 && cp <= 0x08FF) return true;
    if (cp >= 0x0900 && cp <= 0x0FFF) return true;
    if (cp >= 0x0E00 && cp <= 0x18AF) return true;
    if (cp >= 0x2E80 && cp <= 0x31EF) return true;
...
static bool is_contraction(const char* text, size_t i, size_t len, size_t* out_len)
{
    ...
    if (i + 2 < len) 
    {
        char c1 = text[i + 1]; char c2 = text[i + 2];
...
static char* translate_to_gpt2_unicode(const char* raw, size_t length) 
{
    ...
    {
        const char* unicode_seq = get_unicode_for_byte((unsigned char)raw[i]); // Hooks into byte_encoder module natively
...
SplitResult* gpt2_split_text(const char* text) 
{
    ...
    while (i < len) 
    {
        ...
        if (text[i] == ' ') 
        {
            ...
            {
                // Standalone space separation boundary rule
            } 
            else 
            {
                i++;
        ...
        {
            size_t next_idx = i;
            unsigned int cp = decode_utf8(text, len, &next_idx);
            ...
            if (is_unicode_letter(cp)) 
            {
                ...
                while (i < len) 
                {
                    ...
                    if (is_unicode_letter(next_cp)) i = temp_index;
                    else break;
                }
            ...
            else if (is_unicode_digit(cp)) 
            {
                ...
                while (i < len) 
                {
                    size_t temp_index = i;
                    unsigned int next_cp = decode_utf8(text, len, &temp_index);
                    if (is_unicode_digit(next_cp)) i = temp_index;
                    else break;
                }
            ...
            else if (!is_unicode_space(cp)) 
            {
                ...
                while (i < len) 
                {
                    size_t temp_index = i;
                    unsigned int next_cp = decode_utf8(text, len, &temp_index);
                    if (!is_unicode_letter(next_cp) && !is_unicode_digit(next_cp) && !is_unicode_space(next_cp)) 
                    {
                        i = temp_index;
                    } 
                    else 
